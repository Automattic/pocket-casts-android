...
if [ -n "$leftovers" ]; then
  echo "^^^ +++"
  echo "secret.properties still defines:$leftovers — .configure is not on the cleaned-up hash"
  exit 1
fi
...
set +e
SENTRY_AUTH_TOKEN='' ./gradlew --no-daemon --rerun :app:uploadSentryProguardMappingsRelease 2>&1 | tee "$guard_log"
guard_status=${PIPESTATUS[0]}
set -e

if [ "$guard_status" -eq 0 ]; then
  echo "^^^ +++"
  echo "Expected a blank SENTRY_AUTH_TOKEN to fail the build, but it succeeded"
  exit 1
fi

if ! grep -qF "$GUARD_MESSAGE" "$guard_log"; then
  echo "^^^ +++"
  echo "The build failed, but not through the guard: '$GUARD_MESSAGE' is absent from the output"
  exit 1
fi
