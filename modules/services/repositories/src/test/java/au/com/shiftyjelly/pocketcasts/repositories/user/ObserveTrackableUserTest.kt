...
class ObserveTrackableUserTest {
    ...
    @Test
    fun `given user allowed linking crash reports to user, when requesting user details then provide them`() = runTest {
    ...
    @Test
    fun `given user disallowed linking crash reports to user, when requesting user details then return null`() = runTest {
        val mockedUserSettings: UserSetting<Boolean> = mock {
    ...
    @Test
    fun `given user allowed linking crash reports to user but email is not available, when requesting user details then return null`() = runTest {
        val mockedUserSettings: UserSetting<Boolean> = mock {
            on { flow } doReturn MutableStateFlow(true)
        }
        whenever(settings.linkCrashReportsToUser).thenReturn(mockedUserSettings)
