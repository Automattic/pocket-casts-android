...
class EventSink(
    ...
    override fun invoke(event: Trackable) {
        val analyticsEvent = event.toAnalyticsEvent()
...
private fun Trackable.toAnalyticsEvent(): AnalyticsEvent {
    val trackableName = name
    return AnalyticsEvent.entries.first { event ->
        event.key == trackableName
    }
}
