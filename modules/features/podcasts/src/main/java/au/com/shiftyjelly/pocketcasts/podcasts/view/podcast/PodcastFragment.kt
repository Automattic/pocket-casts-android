...
class PodcastFragment : BaseFragment(), Toolbar.OnMenuItemClickListener {
            ...
            AppTheme(theme.activeTheme) {
                if (showTooltip) {
                    Tooltip(
                        ...
                        onDismissRequest = {
                            showTooltip = false
                            binding?.dimScrimView?.visibility = View.GONE
                        },
                    )
                }
            }
    ...
    private fun showTooltipOnEpisodeOptions() {
        lifecycleScope.launch {
            delay(500)
            val positionOfHeader = 2
            val viewHolder = binding?.episodesRecyclerView?.findViewHolderForAdapterPosition(positionOfHeader)
    ...
    fun showTooltipAbove(view: View) {
        ...
        val anchorY = anchorLocation[1] - composeLocation[1] - 200

        tooltipOffset = IntOffset(anchorX, anchorY)
        showTooltip = true
        binding?.dimScrimView?.visibility = View.VISIBLE
    }
